...
The fastest way to set up Auth.js is with an [OAuth](/concepts/oauth) provider. In this tutorial, we'll be setting Auth.js in a web application to be able to sign in with **GitHub**.
...
Sign in to **GitHub**, go to [`Settings / Developers / OAuth Apps`](https://github.com/settings/developers) and click "New OAuth App"
...
Great! We have completed the whole E2E authentication flow setup so that users can sign in to our application through GitHub!
